...
def load_professor_exercises(db: Session, *, professor_id: int, q: str = "") -> list[Exercise]:
    stmt = (
        select(Exercise)
        .where(Exercise.professor_id == professor_id)
        .order_by(Exercise.name.asc(), Exercise.id.desc())
    )
...
def load_professor_routines(db: Session, *, professor_id: int) -> list[Routine]:
    return db.scalars(
        select(Routine)
        .where(Routine.professor_id == professor_id)
        .order_by(Routine.id.desc())
    ).all()


def load_professor_students(db: Session, *, professor_id: int, q: str = "") -> list[User]:
    stmt = (
        select(User)
...
def load_professor_active_assignments(db: Session, *, professor_id: int) -> list[RoutineAssignment]:
    return db.scalars(
        select(RoutineAssignment)
        .join(Routine, Routine.id == RoutineAssignment.routine_id)
        .where(
            Routine.professor_id == professor_id,
